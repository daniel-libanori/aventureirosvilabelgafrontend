    ...
    <Background editchapter={true}>
      ...
      {isOpenExpPoint && (
        <AddExplorationPointModal
          ...
          y={Math.ceil(selectedSquare / colunasMap)}
          expPointArr={expPointArr}
          isOpen={isOpenExpPoint}
          onOpen={onOpenExpPoint}
          onClose={() => {
      ...
      >
        ...
        >
          ...
          <Flex direction="column" w="100%" justify="center">
                ...
                <Flex position="relative">
                  {contarAteNumero(linhasMap).map((num) => (
                    <Flex
                      ...
                      key={num}
                      zIndex={3}
                      h={imageHeight / linhasMap}
                      display="flex"
                      align="center"
                      left={-6}
                      top={(num - 1) * (imageHeight / linhasMap)}
                    >
                  ...
                  <Flex h={imageHeight} w={imageWidth} wrap="wrap" zIndex={2}>
                    {contarAteNumero(linhasMap * colunasMap).map((num) => (
                      <Flex
                        h={imageHeight / linhasMap}
                        w={imageWidth / colunasMap}
                        border="2px solid red"
                        bgColor={
                          selectedSquare == num ? "rgba(255,255,0,0.5)" : null
                        }
                        zIndex={2}
                        align="center"
                        justify="center"
            ...
            >
              {!!selectedSquare &&
                expPointArr?.some((elm) => selectedSquare in elm) &&
                expPointArr[
                  expPointArr
                    .map((e) => parseInt(Object.keys(e)[0]))
                    .indexOf(selectedSquare)
                ][selectedSquare].map((e) => (
                  <Flex
                    key={e.id}
                    border="1px solid black"
                    borderRadius={10}
                    m={5}
                    onClick={() =>
                      handleSelectExplorationPointToOpenUpdateModal(e)
                    }
                    direction="row"
                    align="center"
                  ...
                  >
                    <Flex cursor={"pointer"}>
                      <Text fontSize={25} pt={5} pb={5} pl={5} fontWeight={600}>
                        {e.code}
                      </Text>
                      {!!e.name && (
                        <Text fontSize={25} pt={5} pb={5} pl={1}>
                          - {e.name}
                        </Text>
                      )}
                    </Flex>
              ...
              {!!selectedSquare && (
                <Flex
                  ...
                  onClick={onOpenExpPoint}
                  borderRadius={10}
                  m={5}
                  bgColor="rgba(0,255,0,0.2)"
                >
                  <Text fontSize={25} p={5}>
                    Adicionar Novo Ponto de Exploração
                  </Text>
                </Flex>
              )}
              {!selectedSquare && <Text>Selecione um ponto do mapa</Text>}
            </Flex> */}
          </Flex>
        </Flex>
      </Card>
