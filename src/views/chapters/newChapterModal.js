  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      <ModalContent>
        <ModalHeader>
          {type === "add"
            ? "Criação de Novo Capítulo"
            : `Edição do Capítulo ${selectedChapter.name}`}
        </ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          {creationStep === 1 && type === "add" && (
            <>
              ...
              <Input
                placeholder="Insira aqui o nome do seu novo capítulo..."
                value={chapterName}
          ...
          {creationStep === 2 && type === "add" && (
            <>
              <Text>Para cada capítulo, é necessário um mapa.</Text>
              <Text mt={5}>
          ...
          {type === "update" && (
            <>
              ...
              <Input
                placeholder="Insira aqui o nome do seu capítulo..."
                value={chapterName}
                onChange={(e) => setChapterName(e.target.value)}
              />
            </>
